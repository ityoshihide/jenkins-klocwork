    ...
    stage('Export issues JSON -> SARIF (for Warnings NG)') {
      steps {
        ...
        powershell '''
          ...
          if (!(Test-Path $jsonPath)) { Write-EmptySarif $sarifPath; exit 0 }

          $raw = Get-Content -Path $jsonPath -Raw
          if ([string]::IsNullOrWhiteSpace($raw)) { Write-EmptySarif $sarifPath; exit 0 }
          ...
          if ($null -eq $issues) { Write-EmptySarif $sarifPath; exit 0 }
          ...
          foreach ($it in $issues) {
            ...
            $line = 1
            if ($it.line -and [int]$it.line -gt 0) { $line = [int]$it.line }
